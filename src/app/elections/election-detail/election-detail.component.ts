...
@Component({
  ...
  styleUrls: ['./election-detail.component.styl']
})
export class ElectionDetailComponent implements OnInit {
  ...
  constructor(
    ...
    this.electionLocation = '';

    this.assemblies$ = EMPTY;
    this.parties$ = EMPTY;
    this.electorates$ = EMPTY;
    this.candidates$ = EMPTY;

    this.candidates$ = EMPTY;
    this.assembliesMatches$ = EMPTY;
    this.partiesMatches$ = EMPTY;
    this.electoratesMatches$ = EMPTY;
    this.candidatesMatches$ = EMPTY;

    this.assembliesDisplay$ = EMPTY;
    this.partiesDisplay$ = EMPTY;
    this.electoratesDisplay$ = EMPTY;
  ...
  ngOnInit() {
    ...
    this.election$ = this.route.paramMap.pipe(
      switchMap((params: ParamMap) => {
        const code = params.get('election_code');
        return code ? this.service.election(code) : EMPTY;
      })
    );

    this.election$.subscribe(election => {
      this.electionCode = election.code;
      this.electionDate = election.held;
